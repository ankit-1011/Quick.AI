...
const RemoveObject = () => {
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      const { data } = await axios.post('/api/ai/remove-image-objcet', formData, { headers: { Authorization: `Bearer ${await getToken()}` } })
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex flex-col lg:flex-row items-start gap-4 text-slate-700'>
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <button disabled={loading} className='w-full flex justify-center items-center gap-2 bg-gradient-to-r from-[#417DF6] to-[#8E37EB] text-white px-4 py-2 mt-6 text-sm rounded-1g cursor-pointer'>
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96 '>
...

        {
          ...
          <div className='text-sm flex flex-col items-center gap-5 text-gray-400'>
            ...
            <img src={content} alt='image' className='nt-3 w-full h-full'/>
          )
        }
      </div>
    </div >
